...
@section('title', __('messages.student_register_title'))
...
<div class="auth-wrapper">
    <div class="auth-card">
        ...
        <h2>{{ __('messages.student_register_heading') }}</h2>
        ...
        <form method="POST" action="{{ route('student.register') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" placeholder="{{ __('messages.full_name_ph') }}" value="{{ old('name') }}" required>
                @error('name')<div class="error-text">{{ $message }}</div>@enderror
                <div class="helper"><i class="fas fa-user"></i> {{ __('messages.full_name_helper') }}</div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="helper"><i class="fas fa-envelope"></i> {{ __('messages.email_helper_register') }}</div>
            </div>

            <div class="form-group">
                <label>{{ __('messages.password_label') }} <span class="required">*</span></label>
                <input type="password" name="password" placeholder="{{ __('messages.password_ph') }}" required minlength="8">
                @error('password')<div class="error-text">{{ $message }}</div>@enderror
                <div class="helper"><i class="fas fa-lock"></i> {{ __('messages.password_min_helper') }}</div>
            </div>
            ...
            <button type="submit" class="btn-submit">{{ __('messages.btn_register') }}</button>

            <div class="form-footer">
                <p>
                    {{ __('messages.already_have_account') }}
                    <a href="{{ route('student_login_page') }}">{{ __('messages.login_here') }}</a>
                </p>
            </div>
