...
@section('title', __('messages.student_login_title'))
...
<div class="auth-wrapper">
    <div class="auth-card">
        ...
        <h2>{{ __('messages.student_login_heading') }}</h2>
        ...
        @if(session('error_logout'))
            <div class="alert-error-box">{{ session('error_logout') }}</div>
        @endif
        ...
        <form action="{{ route('student.login') }}" method="post">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" placeholder="{{ __('messages.email_ph') }}" required autocomplete="email" value="{{ old('email') }}">
                @error('email')<div class="error-text">{{ $message }}</div>@enderror
                <div class="helper"><i class="fas fa-envelope"></i> {{ __('messages.email_helper') }}</div>
            </div>

            <div class="form-group">
                <label>{{ __('messages.password_label') }} <span class="required">*</span></label>
                <input type="password" name="password" placeholder="{{ __('messages.password_ph') }}" required autocomplete="current-password">
                @error('password')<div class="error-text">{{ $message }}</div>@enderror
                <div class="helper"><i class="fas fa-lock"></i> {{ __('messages.password_helper') }}</div>
            </div>

            <button type="submit" class="btn-submit">{{ __('messages.btn_login') }}</button>
            ...
            <a href="{{ route('student.google.redirect') }}" class="google-login-btn" aria-label="{{ __('messages.google_login') }}">
                ...
                <span class="google-login-btn__text">{{ __('messages.google_login') }}</span>
            </a>

            <div class="form-footer">
                <p>{{ __('messages.no_account') }} <a href="{{ route('student_register_page') }}">{{ __('messages.create_account') }}</a></p>
            </div>
        </form>
    </div>
</div>
